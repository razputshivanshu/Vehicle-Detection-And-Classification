...
def labels_to_class_weights(labels, nc=80):
    ...
    # classes = labels[:, 0].astypeint  # labels = [class xywh]
    classes = labels[:, 0].astype(int)  # Correct
...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    ...
    class_counts = np.array([np.bincount(x[:, 0].astypeint, minlength=nc) for x in labels])
